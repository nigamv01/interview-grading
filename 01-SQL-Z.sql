...
CREATE OR REPLACE TEMP VIEW workloads
AS SELECT * FROM JSON.`/databricks-coding-challenge/workloads`
...
SELECT DISTINCT CAST(metadata.workloadMetadata.workspaceId as LONG) AS workspaceId
FROM workloads
...
SELECT COUNT(DISTINCT metadata.workloadMetadata.clusterId)
FROM workloads
...
SELECT
  ...
  COUNT_IF(metadata.clusterMetadata.containerIsSpot) as spotNodes,
  COUNT_IF(NOT metadata.clusterMetadata.containerIsSpot) as onDemandNodes
FROM workloads
...
SELECT date, metadata.runtimeMetadata.sparkVersion, SUM(nodeHours)
FROM workloads
WHERE workloadType = 'interactive'
GROUP BY date, metadata.runtimeMetadata.sparkVersion
ORDER BY date, metadata.runtimeMetadata.sparkVersion ASC
...
SELECT *
FROM tpc_h
QUALIFY ROW_NUMBER() OVER (PARTITION BY partKey ORDER BY shipDate DESC) <= 2
